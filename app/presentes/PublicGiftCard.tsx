...
export default function PublicGiftCard({ gift, onClick }: PublicGiftCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group relative w-full text-left bg-warm-white rounded-[22px] border overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_56px_rgba(74,48,40,0.12)] focus:outline-none focus-visible:ring-2 focus-visible:ring-rose/50 ${
        isFull
    ...
    >
      ...
      {/* Image area */}
      <div className="relative w-full aspect-[4/3] bg-gradient-to-br from-gold-light via-blush to-rose/35 flex items-center justify-center overflow-hidden">
        <span
        ...
        {isFull && (
          <div className="absolute inset-0 bg-brand-dark/40 flex items-center justify-center">
            <span className="font-cormorant italic text-white text-xl font-light border border-white/40 px-5 py-1.5 rounded-full backdrop-blur-sm">
              Escolhido ♡
            </span>
          </div>
        )}
        ...
        {/* Donor avatars */}
        {gift.donors.length > 0 && !isFull && (
          <div className="absolute bottom-3 right-3 flex -space-x-1.5">
            {gift.donors.slice(0, 3).map((d, i) => (
              <div
                key={i}
      ...
      <div className="p-4 pb-5">
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          <div className="text-right">
            {isFull ? (
              ...
              <span className={`text-[0.68rem] font-light ${remaining <= 1 ? "text-terracotta" : "text-sage"}`}>
        ...
        {!isFull && (
          <div className="mt-3 flex items-center gap-1.5 text-[0.7rem] font-light tracking-[0.1em] uppercase text-rose opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
